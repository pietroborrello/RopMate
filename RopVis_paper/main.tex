...
While previous tools like, ropper \cite{ropper} or ROPGadget \cite{ROPGadget}, do an extraordinary job in finding useful gadgets, they lack totally of user friendlyness. They present the chain builder with a plain text dump of gadget found, that is meant to be parsed by and or scripting. While this is useful for other automatic tools, presenting the users with thousands of lines of gadgets doesn't help him in any way on the choice of the right gadget. Our work focuses on making the life of the exploiter easier.
...
\paragraph{ROP Gadget Finders.} There exists a plethora of tools to find executable gadgets (sequence of executable and correct instructions that ends with a {\tt ret} instruction): all of them provide the attacker with a list of gadgets among wich to choose \cite{ropper} \cite{ROPGadget}. 
...
\paragraph{Visual ROPChain Builder.} At the time of the paper we weren't able to find any visual or textual tool, that would help an attacker to build his ROPChain. Current tecniques just rely on python scripting.
...
\subsection{Gadgets Dataset} The system takes ad input a dataset of semantically analysed gadgets, provided by a backend server. The backend server essentially takes the original binary collects the gadget through some existing gadged finding tool, and performs a semantic analysis on each found gadget to set the tyoe and the attributes described in Table~\ref{tab:gad_attr}.

\iffalse
\bigskip
The gadgets in the main view are hierarchically ordered by class, and then by parameters. The histogram, on the right of the main view, represents the number of gadgets of each class, to quickly give an hint on the overall view.
